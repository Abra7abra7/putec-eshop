-- Migration: Fix tasting session times v3
-- Date: 2025-08-24

-- Drop existing function and recreate with correct interval handling
...
BEGIN
  ...
  LOOP
    ...
    IF EXTRACT(DOW FROM session_date) != 1 THEN
      ...
      FOR session_time_str IN SELECT unnest(ARRAY['10:00', '14:00', '18:00'])
      LOOP
        ...
        LOOP
          ...
          VALUES (
            package_record.id,
            (session_date || ' ' || session_time_str || ':00')::timestamp with time zone,
            (session_date || ' ' || session_time_str || ':00')::timestamp with time zone + (package_record.duration_minutes || ' minutes')::interval,
